...
export default function GuestCTA({ variant = 'default' }: GuestCTAProps) {
  if (variant === 'inline') {
    return (
      <div className="rounded-3xl border border-amber-200/70 bg-gradient-to-br from-amber-50 via-white to-orange-50 px-5 py-4 shadow-[0_18px_50px_-28px_rgba(180,83,9,0.32)]">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="max-w-xl">
            ...
            <h3 className="text-base font-bold leading-snug text-zinc-900">내 사주에 해당되는지 바로 확인해 보세요</h3>
            <p className="mt-1 text-sm leading-relaxed text-zinc-600">로그인 후 생년월일 1회 입력이면 충분합니다.</p>
          </div>
  ...
  return (
    <div className="overflow-hidden rounded-[28px] border border-amber-200/70 bg-[radial-gradient(circle_at_top_right,_rgba(251,191,36,0.18),_transparent_28%),linear-gradient(135deg,_rgba(255,251,235,0.98),_rgba(255,255,255,1)_42%,_rgba(255,247,237,0.96))] p-6 shadow-[0_24px_70px_-34px_rgba(180,83,9,0.38)]">
      <div className="flex flex-col gap-4">
        <div>
          <p className="mb-2 text-[11px] font-semibold uppercase tracking-[0.24em] text-amber-700">회원 전용 맞춤 판정</p>
          <h3 className="text-[1.95rem] font-bold leading-tight tracking-tight text-zinc-950">이 글이 내 사주에 해당되는지 확인해 보세요</h3>
        </div>

        <p className="max-w-xl text-[15px] leading-7 text-zinc-600">로그인 후 생년월일 1회 입력이면 충분합니다.</p>
