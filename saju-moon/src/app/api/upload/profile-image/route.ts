...
export async function POST(request: NextRequest) {
  ...
  const ipScope = forwardedFor?.split(',')[0]?.trim() || 'unknown'
  const rateLimitResult = await enforceRateLimit(supabase, 'profile_image_upload', user.id, ipScope)
  ...
  if (file.size > MAX_SIZE) {
    return NextResponse.json(
      { error: '프로필 이미지는 2MB 이하만 업로드할 수 있습니다.' },
      { status: 400 },
    )
  }
